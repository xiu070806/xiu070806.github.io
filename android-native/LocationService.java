...
public class LocationService extends Service {
    ...
    private void requestUpdates() {
        ...
        try {
            if (fine || coarse) {
                locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 1000L, 1.0f, listener, Looper.getMainLooper());
            }
            if (locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER)) {
                locationManager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 2000L, 2.0f, listener, Looper.getMainLooper());
            }
        } catch (Exception e) {
            sendError(e.getMessage() == null ? "Không thể khởi động GPS" : e.getMessage());
        }
    }
    ...
    private void saveLocation(Location l) {
        getSharedPreferences(PREFS, MODE_PRIVATE).edit()
            ...
            .putFloat(KEY_ACC, l.hasAccuracy() ? l.getAccuracy() : 0f)
            .putFloat(KEY_SPEED, l.hasSpeed() ? l.getSpeed() : -1f)
            .putFloat(KEY_HEADING, l.hasBearing() ? l.getBearing() : -1f)
    ...
    private void createChannel() {
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel c = new NotificationChannel(
                CHANNEL_ID, "TAXIMET PRO GPS", NotificationManager.IMPORTANCE_LOW
            );
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        if (locationManager == null) requestUpdates();
        return START_STICKY;
    }
